...
export function LoginForm() {
  ...
  useEffect(() => {
    // Check if Supabase redirected a password recovery here instead of /reset-password
    ...
    const rememberedEmail = localStorage.getItem("lifelead-remembered-email");
    if (rememberedEmail) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (rememberMe) {
        localStorage.setItem("lifelead-remembered-email", email);
      } else {
        localStorage.removeItem("lifelead-remembered-email");
      }
        ...
        {isInitializing ? (
          ...
          <div className="w-full flex flex-col items-center animate-in fade-in duration-500">
            ...
            <h1 className="text-[24px] font-extrabold text-[#133020] dark:text-white tracking-tight text-center mb-1.5 transition-colors duration-300">
              {isForgotPasswordView ? "Reset Password" : "Log in to LifeLead"}
            </h1>
            <p className="text-[13px] text-gray-500 dark:text-gray-400 text-center leading-relaxed max-w-[270px] mb-5 transition-colors duration-300">
              {isForgotPasswordView ? "Enter your email address and we will send you a password reset link." : "Enter your credentials to continue monitoring and managing potential clients."}
            </p>
            ...
            <form onSubmit={handleSubmit} className="w-full flex flex-col gap-4">
                ...
                <>
                  ...
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <button
                      ...
                      className="text-[#046241] dark:text-[#ffb347] font-bold hover:underline transition-all cursor-pointer"
                    >
                      Forgot password?
                    </button>
                  </div>
              ...
              >
                {isLoading 
                  ? (isForgotPasswordView ? "Sending..." : "Logging in...") 
                  : (isForgotPasswordView ? "Send Reset Link" : "Log in")}
                {!isLoading && !isForgotPasswordView && (
              ...
              {isForgotPasswordView && (
                <button
                  ...
                  className="mt-2 text-xs font-bold text-gray-500 hover:text-gray-800 dark:hover:text-gray-200 transition-colors cursor-pointer text-center"
                >
                  Back to log in
                </button>
              )}
